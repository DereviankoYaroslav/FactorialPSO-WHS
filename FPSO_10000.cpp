...
int *particleSwarmOptimization(int size, int count, int N, int maxIter, int *finalIter);
...
int main()
{
   ...
   int *ar = particleSwarmOptimization(256, 8, N, maxIter, &finalIter);
   ...
   /*long long costWHS = costFunctionWHS(aesSbox,256,8);
   cout << "cost by WHS = " << costWHS;*/
   return 0;
}
...
int *particleSwarmOptimization(int size, int count, int N, int maxIter, int *finalIter){
    ...
    long long minCost = costFunctionWHS(population[0],size,count);
    for (int q = 0; q < N; ++q){
      	for(int w = 0; w < size; ++w){
            printf("%d ",population[q][w]);
        }
        int LAT = LATMax(population[q],size,count);
        int NL = raiseToPower(2, count - 1) - LAT;
        long long cost = costFunctionWHS(population[q],size,count);
        printf( "\nNon-linearity from LAT = %d \n", NL);
        printf("\n");
        pBestNL[q] = NL;
        pBestCost [q] = cost;
    ...
    int *sub = (int *) calloc (256, sizeof(int));
   	...
   	cout << "p  = " <<p[i];
    	cout << "\n";
    	/*ZZ *arr = numberToFactorial(pRes,256);
	int *factNum = (int *) calloc (256,sizeof(int));
	//cout << "\n\nFACTORIAL NUMBER\n";
	for (int i = 0; i < 256 ; ++i){
	  conv(factNum[i],arr[i]);
	...
	int LAT3 = LATMax(subFin, size, count);
        ...
        printf("\nNon-linearity = %d \n", NL3);
        long long cost = costFunctionWHS(subFin,size,count);
        cout << "\nCost = "<< cost << "\n";
        if (cost <= pBestCost[i]){
           if (cost <= gBestCost[0]){
           	gBestNL[0] = NL3;
    ...
    /*ZZ *arr = numberToFactorial(gRes,256);
	int *factNum = (int *) calloc (256,sizeof(int));
	//cout << "\n\nFACTORIAL NUMBER\n";
	for (int i = 0; i < 256 ; ++i){
	  conv(factNum[i],arr[i]);
    ...
    */
    ...
    /*for (int h = 0; h < N; ++h) {
   	...
   	ZZ result = factorialNumberToNumber(factorialNumber,256);
   	ZZ minus;
   	//minus = conv<ZZ>(999999999999+3313213);
   	cout << "\n\nNumber = \n";
   	cout << result-minus;
   	cout << "\n";
   	ZZ *arr = numberToFactorial(result,256);
	int *factNum = (int *) calloc (256,sizeof(int));
	//cout << "\n\nFACTORIAL NUMBER\n";
	for (int i = 0; i < 256 ; ++i){
	  conv(factNum[i],arr[i]);
	...
	int LAT3 = LATMax(subFin, size, count);
        int NL3 = raiseToPower(2, count - 1) - LAT3;
        printf("\nNon-linearity = %d \n", NL3);
        int ucCheck3 = linearRedundancy(subFin, 256, 8);
        int ai3 = algebraicImmunity(subFin, 256, 8);
        int du3 = deltaUniformity(subFin, 256, 8);
